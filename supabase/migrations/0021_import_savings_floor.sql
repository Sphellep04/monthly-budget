-- Extends import_all_data (0003, 0007, 0009, 0011, 0013, 0015, 0017, 0019) so
-- restoring a backup keeps the savings floor setting.
...
begin
  ...
  delete from category_rules where owner = v_owner;
  ...
  insert into expenses (owner, budget_id, date, amount_cents, notes, receipt_url, recurring_template_id, split_group_id, created_at)
  ...
  insert into category_rules (owner, keyword, category, created_at)
  select v_owner, r->>'keyword', r->>'category', coalesce((r->>'created_at')::timestamptz, now())
  from jsonb_array_elements(coalesce(payload->'categoryRules', '[]'::jsonb)) r;

  if payload ? 'userSettings' then
    insert into user_settings (owner, alert_threshold_percent, savings_floor_cents)
    values (
      v_owner,
      coalesce((payload->'userSettings'->>'alert_threshold_percent')::int, 80),
      coalesce((payload->'userSettings'->>'savings_floor_cents')::bigint, 0)
    )
    on conflict (owner) do update set
      alert_threshold_percent = excluded.alert_threshold_percent,
      savings_floor_cents = excluded.savings_floor_cents;
  end if;
end;
$$;
