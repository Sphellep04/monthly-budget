...
export function RecurringIncomeForm({
  ...
  const existing = templates.find((t) => t.id === incomeId) ?? null;
  ...
  const isPending =
    createRecurringIncome.isPending || updateRecurringIncome.isPending;
    ...
    if (isEditing && incomeId !== null) {
      updateRecurringIncome.mutate(
        { id: incomeId, input },
        {
          ...
          onError: () => toast.error("Failed to update recurring income"),
        },
      );
    } else {
      createRecurringIncome.mutate(input, {
        onSuccess: () => {
          toast.success("Recurring income added");
          onOpenChange(false);
        },
        onError: () => toast.error("Failed to create recurring income"),
      });
    }
  }
      ...
      <DialogContent className="sm:max-w-md shadow-premium">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            {isEditing
              ? "Update this recurring income source."
              : "This income will be auto-logged each month on the day you choose."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          {/* Source */}
          <div>
            <FieldLabel htmlFor="ri-source">Source</FieldLabel>
            <Input
              id="ri-source"
              placeholder="e.g. Salary, Freelance retainer"
              value={source}
              onChange={(e) => setSource(e.target.value)}
              autoFocus
              className={`input-focus h-10 ${errors.source ? "border-destructive" : ""}`}
            />
            {errors.source && (
              <p className="text-xs text-destructive mt-1">{errors.source}</p>
            )}
          </div>

          {/* Amount */}
          <div>
            <FieldLabel htmlFor="ri-amount">Amount</FieldLabel>
          ...
          {/* Day of month */}
          <div>
            <FieldLabel htmlFor="ri-day">Day of Month</FieldLabel>
            <Input
              id="ri-day"
              type="number"
              inputMode="numeric"
            ...
            {errors.day ? (
              ...
              <p className="text-xs text-muted-foreground mt-1.5">
                For months with fewer days, the income will be logged on the
                last available day.
              </p>
            )}
          </div>
